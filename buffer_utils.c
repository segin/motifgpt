...
void init_assistant_response_buffer(assistant_response_buffer_t *arb, size_t initial_capacity) {
    arb->capacity = initial_capacity;
    arb->length = 0;
    arb->buffer = malloc(arb->capacity);
    if (arb->buffer) {
        arb->buffer[0] = '\0';
    }
}
...
void append_to_assistant_buffer(assistant_response_buffer_t *arb, const char* text) {
    ...
    if (arb->length + len + 1 > arb->capacity) {
        arb->capacity = (arb->length + len + 1) * 2;
        char *new_buf = realloc(arb->buffer, arb->capacity);
        if (!new_buf) {
            perror("realloc assistant_buffer");
            if (arb->buffer) free(arb->buffer);
            arb->buffer = NULL;
            arb->length = 0;
            arb->capacity = 0;
            return;
    ...
    memcpy(arb->buffer + arb->length, text, len);
    arb->length += len;
    arb->buffer[arb->length] = '\0';
}
