...
void add_message_to_history(dp_message_role_t role, const char* text_content, const char* img_mime_type, const char* img_base64_data) {
    ...
    if (chat_history_count >= effective_max_history && effective_max_history > 0 && !history_limits_disabled ) {
        ...
        if (messages_to_remove > 0) {
             ...
             dp_free_messages(chat_history, messages_to_remove);
             int remaining_count = chat_history_count - messages_to_remove;
             if (remaining_count > 0) memmove(chat_history, &chat_history[messages_to_remove], remaining_count * sizeof(dp_message_t));
             chat_history_count = remaining_count;
        }
    }
    if (chat_history_count >= chat_history_capacity) {
        ...
        if (!new_history) { perror("realloc chat_history"); pthread_mutex_unlock(&history_mutex); return; }
