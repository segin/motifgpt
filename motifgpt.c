...
void load_settings() {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (key && value) {
            ...
            } else if (strcmp(key, KEY_GEMINI_API_KEY) == 0) strncpy(current_gemini_api_key, value, sizeof(current_gemini_api_key)-1);
            else if (strcmp(key, KEY_GEMINI_MODEL) == 0) strncpy(current_gemini_model, value, sizeof(current_gemini_model)-1);
            else if (strcmp(key, KEY_OPENAI_API_KEY) == 0) strncpy(current_openai_api_key, value, sizeof(current_openai_api_key)-1);
            else if (strcmp(key, KEY_OPENAI_MODEL) == 0) strncpy(current_openai_model, value, sizeof(current_openai_model)-1);
            else if (strcmp(key, KEY_OPENAI_BASE_URL) == 0) strncpy(current_openai_base_url, value, sizeof(current_openai_base_url)-1);
            else if (strcmp(key, KEY_ANTHROPIC_API_KEY) == 0) strncpy(current_anthropic_api_key, value, sizeof(current_anthropic_api_key)-1);
            else if (strcmp(key, KEY_ANTHROPIC_MODEL) == 0) strncpy(current_anthropic_model, value, sizeof(current_anthropic_model)-1);
            else if (strcmp(key, KEY_MAX_HISTORY) == 0) current_max_history_messages = atoi(value);
            else if (strcmp(key, KEY_SYSTEM_PROMPT) == 0) strncpy(current_system_prompt, value, sizeof(current_system_prompt)-1);
