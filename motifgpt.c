...
static void get_setting_value(Widget w, char *dest, size_t dest_size, const char *placeholder, Boolean clear_if_placeholder) {
    ...
    if (clear_if_placeholder && strcmp(tmp, placeholder) == 0 && fg_color == grey_fg_color) {
        dest[0] = '\0';
    } else {
...
void retrieve_settings_from_dialog() {
    ...
    get_setting_value(gemini_api_key_text, current_gemini_api_key, sizeof(current_gemini_api_key), DEFAULT_GEMINI_KEY_PLACEHOLDER, True);
    get_setting_value(gemini_model_text, current_gemini_model, sizeof(current_gemini_model), DEFAULT_GEMINI_MODEL, False);

    get_setting_value(openai_api_key_text, current_openai_api_key, sizeof(current_openai_api_key), DEFAULT_OPENAI_KEY_PLACEHOLDER, True);
    get_setting_value(openai_model_text, current_openai_model, sizeof(current_openai_model), DEFAULT_OPENAI_MODEL, False);
    get_setting_value(openai_base_url_text, current_openai_base_url, sizeof(current_openai_base_url), DEFAULT_OPENAI_BASE_URL, True);

    get_setting_value(anthropic_api_key_text, current_anthropic_api_key, sizeof(current_anthropic_api_key), DEFAULT_ANTHROPIC_KEY_PLACEHOLDER, True);
    get_setting_value(anthropic_model_text, current_anthropic_model, sizeof(current_anthropic_model), DEFAULT_ANTHROPIC_MODEL, False);
