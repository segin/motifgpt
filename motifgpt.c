...
assistant_response_buffer_t assistant_response_buffer;
...
int stream_handler(const char* token, void* user_data, bool is_final, const char* error_during_stream) {
    if (error_during_stream) {
        ...
        if (assistant_response_buffer.buffer) assistant_response_buffer.buffer[0] = '\0';
        assistant_response_buffer.length = 0;
        return 1;
    }
    ...
    if (token) {
        append_to_assistant_buffer(&assistant_response_buffer, token);
    ...
    if (nbytes == sizeof(pipe_message_t)) {
        switch (msg.type) {
            ...
            case PIPE_MSG_STREAM_END:
                if (assistant_is_replying && !prefix_already_added_for_current_reply && assistant_response_buffer.length == 0) {
                    append_to_conversation(current_assistant_prefix);
                }
                append_to_conversation("\n");
                if (assistant_response_buffer.buffer && assistant_response_buffer.length > 0) {
                    add_message_to_history(DP_ROLE_ASSISTANT, assistant_response_buffer.buffer, NULL, NULL);
                } else if (assistant_is_replying && assistant_response_buffer.length == 0) {
...
void quit_callback(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    free_assistant_response_buffer(&assistant_response_buffer);
...
int main(int argc, char **argv) {
    ...
    init_assistant_response_buffer(&assistant_response_buffer, 1024);
    if (!assistant_response_buffer.buffer) { return 1; }
